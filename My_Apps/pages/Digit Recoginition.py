...
# ============================================================
# 1. Load trained digit model
# ============================================================

@st.cache_resource
def load_digit_model():

    # Main My_Apps project folder
    BASE_DIR = Path(__file__).resolve().parent.parent

    # Your train.py saves the model as digit_model.keras
    model_path = BASE_DIR / "digit_model.keras"

    if not model_path.exists():
        st.error(
            f"Model file not found:\n\n{model_path}\n\n"
            "Please run train.py first so that digit_model.keras "
            "is created inside the main My_Apps folder."
        )
        st.stop()
...
def preprocess_digit_image(img):
    ...
    """
    Preprocesses an image for digit recognition:
...
class RealTimeDigitRecognizer(VideoProcessorBase):
    ...
    def recv(self, frame):
        ...
        # Convert incoming video frame to BGR
        ...
        confidence = (
            float(prediction[predicted_digit]) * 100
        )
...
if input_mode == "Real-Time Live Camera 📹":
    ...
    st.info(
        ...
        "The model will continuously draw predictions "
        "over the video."
    )
...
# ============================================================
    ...
    if uploaded_file is not None:
        ...
        if img is None:
            ...
            confidence = (
                float(prediction[predicted_digit])
                * 100
            )
